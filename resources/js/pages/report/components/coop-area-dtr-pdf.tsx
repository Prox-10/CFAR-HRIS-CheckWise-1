...
export default function CoopAreaDTRPDF({ reportDate, data }: CoopAreaDTRPDFProps) {
    const titleDate = format(reportDate, 'MMMM dd, yyyy');

    // Render a table row
    const renderTableRow = (index: number, employee?: CoopAreaEmployee) => {
    ...
    // Render a table
    const renderTable = (employees: CoopAreaEmployee[], maxRows: number) => {
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerCenter}>
                        ...
                        <Text style={styles.areaName}>COOP AREA DTR</Text>
                    </View>
                    <View style={styles.headerRight} />
                </View>

                {/* COOP AREA Table */}
                <View style={styles.tableContainer}>{renderTable(data.coopArea || [], 14)}</View>

                {/* ENGEERING GROUP Section */}
                <Text style={styles.sectionTitle}>ENGEERING GROUP</Text>
                <View style={styles.tableContainer}>{renderTable(data.engineeringGroup || [], 3)}</View>

                {/* Checked By */}
                <View style={styles.checkedBy}>
                    <Text style={styles.checkedByLabel}>Checked By:</Text>
                    <View style={styles.checkedByLine} />
                </View>
            </Page>
        </Document>
    );
